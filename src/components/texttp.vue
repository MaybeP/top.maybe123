<template>
  <div style="text-align: center">
    <Card>
      ...
      <div v-if="file!=='尚未选择'">已选择图片: {{ file.name }} <Button type="text" @click="upload" :loading="loadingStatus">{{ this.isup==true ? '再次提交' : '提交' }}</Button></div>
  ...
  export default {
    ...
    methods: {
      ...
      upload () {
        ...
        th.$Loading.start()
        this.$axios.post('http://www.maybe123.top:8080/uptext.action?',
          formData, config)
          .then(function (result) {
            // th.file = null
            th.isup = true
            th.loadingStatus = false
            if (result.data.result_num === 0) {
              th.$Message.error('未检查到字符！,请重新选择其它图片！')
              th.tip = '未检查到字符！,请重新选择其它图片！'
            } else {
              th.tip = '当前检测到共：' + result.data.words_result_num
              var str = ''
              for (let i = 0; i < result.data.words_result_num; i++) {
                str += result.data.words_result[i].words + '\r\n'
              }
              sessionStorage.setItem('sb_text', str)
            }
            th.$Loading.finish()
          })
          .catch(
